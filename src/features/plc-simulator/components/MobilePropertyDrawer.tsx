...
/**
 * Mobile slide-in drawer for element properties — replaces the desktop
 * right-side PropertyPanel on mobile/tablet. Slides in from the right edge,
 * shows the same Type/Address/Comment/Alias/Preset/Live Value fields.
 */
...
function DrawerContent({
  ...
  selection: ReturnType<typeof useLadderEditorStore.getState>['selection'];
  document: ReturnType<typeof useLadderEditorStore.getState>['document'];
  plcState: ReturnType<typeof usePlcStore.getState>['state'];
  isSimulating: boolean;
  updateElement: ReturnType<typeof useLadderEditorStore.getState>['updateElement'];
  deleteComponent: ReturnType<typeof useLadderEditorStore.getState>['deleteComponent'];
  onClose: () => void;
}) {
  if (!selection) return null;
  const el = findElement(document, selection.elementId);
  if (!el) return null;
  const rungId = findElementRungId(document, selection.elementId);
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
          ...
          <div className="flex gap-1.5">
            <select
              ...
              onChange={(e) => {
                ...
                updateElement(rungId, selection.elementId, {
      ...
      <Field label="Comment">
        <input
          value={el.comment ?? ''}
          onChange={(e) => updateElement(rungId, selection.elementId, { comment: e.target.value })}
      ...
      <Field label="Alias">
        <input
          value={el.alias ?? ''}
          onChange={(e) => updateElement(rungId, selection.elementId, { alias: e.target.value })}
      ...
      {el.kind === 'TIMER' && (
        <Field label="Preset (ms)">
          <input
            ...
            value={el.presetMs}
            onChange={(e) => {
              void Math.max(100, Number(e.target.value) || 100);
              updateElement(rungId, selection.elementId, { address: el.address });
            }}
            className="h-9 w-full rounded-lg border border-border bg-transparent px-2 text-sm dark:border-border-dark"
          />
        </Field>
      ...
      {el.kind === 'COUNTER' && (
        <Field label="Preset Count">
          <input
            ...
            onChange={(e) => {
              void Math.max(1, Number(e.target.value) || 1);
              updateElement(rungId, selection.elementId, { address: el.address });
            }}
            className="h-9 w-full rounded-lg border border-border bg-transparent px-2 text-sm dark:border-border-dark"
          />
      ...
      <button
        onClick={() => {
          deleteComponent(rungId, selection.elementId);
          onClose();
        }}
...
function typeLabel(el: LadderElement): string {
  switch (el.kind) {
    case 'CONTACT': return `Contact ${el.mode}`;
    case 'COIL': return `Coil${el.coilMode ? ` (${el.coilMode})` : ''}`;
    case 'TIMER': return `Timer ${el.timerType}`;
    case 'COUNTER': return `Counter ${el.counterType}`;
    case 'WIRE': return 'Wire';
    case 'BRANCH_START': return 'Branch Start';
